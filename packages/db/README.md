...
| 0012_guest_sessions            | `guest_sessions` (checkout identity for a guest: opaque token stored hashed, 24-hour lifetime, the verified email that is their ticket-cap key). No `users` row, no foreign key to one, and never authentication                                                           |
| 0013_guest_email_verifications | `guest_email_verifications` (the six-digit code a guest proves their email with: SHA-256 only, single use, TTL capped at an hour by CHECK, attempts monotonic under the guard trigger). `hv_app` cannot DELETE or TRUNCATE it, so an attempt count cannot be erased        |
